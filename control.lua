...
script.on_event({
    ...
    if entity and entity.valid then
        ...
        elseif flib_table.find(constants.assemble_entities, entity.name) then
            building_control.on_entity_built(entity, event.player_index, event.tags, false)
        elseif flib_table.find(constants.disassemble_entities, entity.name) then
            building_control.on_entity_built(entity, event.player_index, event.tags, true)
        end
    end
end)
...
script.on_event({ defines.events.on_pre_build }, function(event)
    ...
    if blueprint_entities and next(blueprint_entities) then
        ...
        if found_our_entity then
            blueprints.on_player_paste_blueprint_ghosts(blueprint_entities, player.surface, event.direction,
                event.position)
        end
    end
end)
...
script.on_event({ defines.events.on_entity_settings_pasted }, function(event)
    ...
    if flib_table.find(constants.building_entities, source.name) and
        flib_table.find(constants.building_entities, dest.name) then
        local player = game.players[event.player_index]
...
script.on_event({
    ...
    local entity = event.entity
    if entity then
        if flib_table.find(constants.assemble_entities, event.entity.name) then
            gui.create_assembler_gui(game.players[event.player_index], entity)
        elseif flib_table.find(constants.disassemble_entities, event.entity.name) then
            gui.create_disassembler_gui(game.players[event.player_index], entity)
        end
    end
end)
...
script.on_event({
    ...
    if entity then
        ...
        elseif flib_table.find(constants.disassemble_entities, event.entity.name) then
            gui.destroy_disassembler_gui(game.players[event.player_index])
        end
    end
end)
...
script.on_nth_tick(constants.bucket_update_interval_seconds * 60, function(event)
    local bucket = global_data.get_bucket(event.tick)
    if bucket then
        tick.bucket_update(bucket)
    end
end)
